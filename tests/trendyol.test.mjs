...
test("computeRequiredPrice maliyet arttıkça gerekli fiyatı da artırır", () => {
  ...
  const high = computeRequiredPrice(250, 9999);
  assert.ok(high > low);
});

test("computeRequiredPrice maliyet 0 için de kademeli sabit kârı (200 TL altı -> 25 TL) yansıtır", () => {
  // productCostWithVat=0 -> requiredPrice = (0+29+80+25) / (1-efektifKomisyon)
  const result = computeRequiredPrice(0, 9999);
  assert.ok(Math.abs(result - 134 / (1 - effectiveCommissionRateFor(9999))) < 1e-9);
});
...
test("calculateTrendyolLimitsFromCost 200 TL eşiğinin hemen altı ve üstü arasında kâr hedefi sıçrar", () => {
  const belowThreshold = calculateTrendyolLimitsFromCost(199, 9999);
  const atThreshold = calculateTrendyolLimitsFromCost(200, 9999);
...
test("calculateTrendyolLimitsFromCost maliyet arttıkça sınırları da artırır", () => {
  ...
  const high = calculateTrendyolLimitsFromCost(250, 9999);
