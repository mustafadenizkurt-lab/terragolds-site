...
export async function GET(request: Request) {
  ...
  try {
    ...
    const hepsiburadaShippingOrders: AdminShippingOrder[] =
      hepsiburadaOrders.results.map((order) => {
        let parsedItems: AdminShippingOrder["items"] = [];
        try {
          parsedItems = (
            JSON.parse(order.items_json) as {
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    // Aynı tek yönlü kural: sadece Hepsiburada'ya bildirir, D1'deki
    // güncelleme (yukarıda zaten yazıldı) her koşulda geçerli kalır.
    if (table === "hepsiburada_orders" && status === "shipped") {
      try {
        const row = await db
          .prepare(
            "SELECT hepsiburada_package_number AS packageNumber FROM hepsiburada_orders WHERE id = ?",
          )
          .bind(id)
          .first<{ packageNumber: string | null }>();
        if (row?.packageNumber) {
          await fulfillHepsiburadaOrder(
            row.packageNumber,
            shippingCarrier,
            trackingNumber,
          );
        ...
        return Response.json({
          ok: true,
          warning: `Sipariş güncellendi ama Hepsiburada'ya bildirilemedi: ${
            error instanceof Error ? error.message : "bilinmeyen hata"
          }`,
        });
      }
